...
# 1. The Sulfur and Tree_ring datasets are correlated, so combining them would be helpful for data analysis. However, the sulfur data is provided in decimal years, while the tree_ring data is recorded yearly. Therefore, the sulfur data needs to be tidied by averaging the sulfur variables for each year.

sulfur_year <- sulfur %>%
  filter(year >= 500 & year <= 706) %>% 
  mutate(year = floor(year)) %>%          # Round down the year (e.g., 501.999 becomes 501)
